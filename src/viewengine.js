...
function getView(viewName, viewData, callback) {
  fileSystem.readFile(__dirname + "/views/" + viewName, "utf-8", function(fileError, fileData) {
    if (fileError)
      throw fileError;
...
function getLayout(layoutName, view, viewData, callback) {
  fileSystem.readFile(__dirname + "/views/" + layoutName, "utf-8", function(fileError, fileData) {
    if (fileError)
      throw fileError;
